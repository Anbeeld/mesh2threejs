...
export function createCandidate() {
  ...
  const turret = part("turret", new THREE.CylinderGeometry(1.15, 1.3, 0.9, 12));
  ...
  const gun = part("gun", new THREE.CylinderGeometry(0.12, 0.12, 3.4, 8), [0, 0, 1.7]);
  gun.rotation.x = Math.PI / 2;
  gunPivot.add(gun);
  turretPivot.add(gunPivot, part("cupola", new THREE.CylinderGeometry(0.35, 0.4, 0.35, 8), [0.35, 0.6, -0.1], true));
  root.add(turretPivot);
  for (const side of [-1, 1]) {
    for (let index = 0; index < 5; index += 1) {
      const wheel = part(`road-wheel-${side}-${index}`, new THREE.CylinderGeometry(0.55, 0.55, 0.22, 10), [side * 1.55, 0, -2 + index]);
