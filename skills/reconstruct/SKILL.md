...
2. Resolve mode first: a workspace whose state carries `mirrorOfRun` is bound to a trusted run - every operation goes through the broker operation API (`probe`, `onboard-oracle`, `register`, `derive`, `gate`, `lock`, ...); raw CLI mutations are refused there. If no workspace exists and the host can pin goal + oracle, prefer trusted intake (`create-workspace-run` through the broker admin channel); otherwise route with `mesh2threejs route`, initialize it, and record the selected profile/style as decisions.
