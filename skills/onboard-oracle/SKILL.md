...
4. Load the prepared oracle and prove forward/up, scale, ground, major semantics, required pivots, and physical frame (lateral/longitudinal/track-pair/gun-forward) with `verifyOracleRegistration`. Require oracle frame sanity board (front/rear/side/top/perspective) and inspect it — a source whose front image is visibly a side view must not proceed.
