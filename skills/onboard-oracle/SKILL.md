...
Mode resolution first: on a trusted run (`state.mirrorOfRun`), use broker operations (`probe`, `onboard-oracle`, `oracle-sanity`, `register`). On an unbound development workspace, use the CLI equivalents.

1. Run broker `probe` (or `mesh2threejs probe` in development mode); reject malformed, unsupported, or provenance-unknown inputs.
