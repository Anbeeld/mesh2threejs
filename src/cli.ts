  ...
  try {
    ...
    switch (command) {
      ...
      case "status": {
        ...
        if (!path) throw new Error("status requires a state path");
        const state = await loadTaskState(resolve(path));
      ...
      case "next": {
        const path = parsed.positional[0];
        if (!path) throw new Error("next requires a state path");
        const state = await loadTaskState(resolve(path));
      ...
      case "record-evidence": {
        ...
        const statePath = resolve(path);
        const artifactPath = resolve(required(parsed.options, "artifact"));
        const artifact = JSON.parse(await readFile(artifactPath, "utf8")) as EvidenceArtifact;
        const state = recordEvidenceArtifact(await loadTaskState(statePath), artifactPath, artifact);
        await saveTaskState(statePath, state);
      ...
      case "record-review": {
        ...
        const statePath = resolve(path);
        const packet = JSON.parse(await readFile(resolve(required(parsed.options, "packet")), "utf8")) as VisualReviewPacket;
        const verdict = JSON.parse(await readFile(resolve(required(parsed.options, "verdict")), "utf8")) as VisualReviewVerdict;
        ...
        state = recordEvidenceArtifact(state, artifactPath, artifact);
        await saveTaskState(statePath, state);
      ...
      case "register": {
        ...
        if (parsed.options.state || parsed.options.artifact) {
          if (!parsed.options.state || !parsed.options.artifact) throw new Error("registration state recording requires both --state and --artifact");
          const statePath = resolve(parsed.options.state);
          ...
          const artifactPath = resolve(parsed.options.artifact);
          await mkdir(dirname(artifactPath), { recursive: true });
          await writeFile(artifactPath, `${json(artifact)}\n`, { flag: "wx" });
          state = recordEvidenceArtifact(state, artifactPath, artifact);
          await saveTaskState(statePath, state);
      ...
      case "gate": {
        ...
        if (!validateOracleManifest(manifest).valid) throw new Error("oracle manifest schema is invalid");
        const candidate = await loadCandidateRuntime(resolve(required(parsed.options, "candidate")));
        const oracle = await loadPreparedOracle(manifest);
        const profile = required(parsed.options, "profile") as ProfileId;
        if (profile !== "tank" && profile !== "generic") throw new Error("--profile must be tank or generic");
        const subjectContract = parsed.options["subject-contract"] ? JSON.parse(await readFile(resolve(parsed.options["subject-contract"]), "utf8")) as GenericSubjectContract : undefined;
        ...
        if (parsed.options.out) await writeFile(resolve(parsed.options.out), rendered);
        if (parsed.options.state || parsed.options["artifact-dir"]) {
          if (!parsed.options.state || !parsed.options["artifact-dir"]) throw new Error("gate state recording requires both --state and --artifact-dir");
          const statePath = resolve(parsed.options.state);
          let state = await loadTaskState(statePath);
          ...
          const directory = resolve(parsed.options["artifact-dir"]);
      ...
      case "render": {
        ...
        if (!validateOracleManifest(manifest).valid) throw new Error("oracle manifest schema is invalid");
        const oracle = await loadPreparedOracle(manifest);
        const candidateRuntime = await loadCandidateRuntime(resolve(required(parsed.options, "candidate")));
        ...
        const directory = resolve(required(parsed.options, "out-dir"));
        await mkdir(directory, { recursive: true });
        ...
        for (const cameraId of ["side", "front", "plan"] as const) {
          for (const pass of passes) {
            ...
            captures.push({ subject: "oracle", pass, cameraId, path: oraclePath, sha256: sha256(await readFile(oraclePath)) }, { subject: "candidate", pass, cameraId, path: candidatePath, sha256: sha256(await readFile(candidatePath)) });
        ...
        const renderManifest = { schemaVersion: 1, kind: "deterministic-cpu-render-evidence", oracleHash: fingerprintScene(oracle), candidateHash: candidateRuntime.sourceHash ? sha256(canonicalJson({ neutralSceneHash, sourceHash: candidateRuntime.sourceHash })) : neutralSceneHash, frame, profileHash: sha256(canonicalJson(profile)), captures, comparisonBoards: await Promise.all(boards.map(async (path) => ({ path, sha256: sha256(await readFile(path)) }))), turntable: await Promise.all(turntable.map(async (path) => ({ path, sha256: sha256(await readFile(path)) }))) };
        const manifestPath = join(directory, "render-manifest.json");
        await writeFile(manifestPath, `${json(renderManifest)}\n`, { flag: "wx" });
        io.stdout(json({ status: "rendered-deterministic-cpu", manifest: manifestPath, captures: captures.length, turntable: turntable.length }));
        return 0;
      }
      case "workorders": {
      ...
      case "finalize": {
        ...
        const certified = await certifyStateFromArtifacts(await loadTaskState(resolve(statePath)));
